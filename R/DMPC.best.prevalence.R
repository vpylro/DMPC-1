#'Best Prevalence
#'
#'Runs Permanova analysis on Bray-Curtis dissimilarity and performes a classification of variables with RandomForests for each prevalence cutoff returned by DMPC.prevalence()
#'
...
#' DMPC.best.prevalence(prev, "Environment", method.dist="bray")
#' @importFrom phyloseq "otu_table"
#' @importFrom phyloseq "sample_data"
#'@export
DMPC.best.prevalence = function (prev.list, variable, method.dist="bray") {
  set.seed(2125)
  perm=list()
  randon=list()
  ...
  for (i in prev.list){
    ...
    randon[[length(randon)+1]]=train.model$err.rate[500,1]
    print("Done")
  }
  ...
  Cutoff= paste(as.numeric(names(prev.list)), "%", sep = "")
  ...
  OOB.err=cbind(results1,results2,Cutoff,OTUs,Nseqs)
  return(OOB.err)
}
